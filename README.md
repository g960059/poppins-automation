...
- **作成タブの状態表示**: 宛先に加えて、最終メッセージ、入力欄の有無、履歴/予定の取得状態、Google Calendar状態を表示。
...
- 依頼/履歴取得に失敗した場合は、生成結果の上に警告が出る。ログインし直すか、SmartSitterのメッセージ室を再読み込みする。
...
- 宛先ズレ防止の `expectedContext`、既存入力保護、作成タブの状態表示が実装されていること。
